...
public class FlowAdapter {
      ...
      for (int i = 0; i < jsonArray.length(); i++) {
        ...
        commentArray.add(
            new Comment(
                ...
                jsonObject.getString("RIC"),
                jsonObject.getString("belongsTo"),
  ...
  public static String getQuote(String symbol, boolean isAndrewMoffatt, boolean isChalk) {
    ...
    try {
      ...
      if (isChalk) {
        if (last == 0 || last == null || close == 0 || close == null) {
          return symbol;
        } else {
          ret = last / close - 1;
        }
      } else if (close == 0 || close == null) {
        ret = (double) 0;
      } else if (last == 0 || last == null) {
        ret = (bid + ask) / 2 / close - 1;
      } else {
        ret = last / close - 1;
      }
      ...
      fontColor =
          isUp
              ? (isAndrewMoffatt ? "<font color='#008000'>" : "<font color='#27AE60'>")
              : (isAndrewMoffatt ? "<font color='#ed1b2e'>" : "<font color='#C0392B'>");
