...
public class EquityFileParser extends TimerTask {
	...
	private static final Logger logger = Logger
			.getLogger(EquityFileParser.class.getName());
	private static Session session;
	private static ChannelSftp channel;
	public static HashMap<String, TickerResearch> tickerResearchMap = new HashMap<String, TickerResearch>();
	public static List<FlowStories> flowStoryList = new ArrayList<FlowStories>();

	static {
		Calendar today = Calendar.getInstance();
		Timer refreshTimer = new Timer();
		refreshTimer.scheduleAtFixedRate(new EquityFileParser(),
				today.getTime(),
		...
		if (channel != null) {
			try {
				...
				for (Object file : channel.ls("/")) {
					...
					if (filename.contains("coverageList")
							&& filename.endsWith(".csv"))
						fileMap.put(++i, filename);
				}

				if (fileMap.size() > 0) {
					logger.info("Using " + fileMap.get(fileMap.size())
							+ " from SFTP.");
					...
					while (fileIterator.hasNext()) {
						...
						try {
							...
							publishDate.setTime(dateFormatter.parse(record
									.get(7)));
