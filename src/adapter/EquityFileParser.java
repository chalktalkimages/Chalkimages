...
public class EquityFileParser extends TimerTask {
	...
	static {
		...
		refreshTimer.scheduleAtFixedRate(new EquityFileParser(), today.getTime(), TimeUnit.MILLISECONDS.convert(5, TimeUnit.MINUTES));
	}
		...
		if(channel != null){
			try{
				...
				for(Object file : channel.ls("/")){
					...
					if(filename.contains("coverageList") && filename.endsWith(".csv")) fileMap.put(++i, filename);
				};
