...
public class MiningScoopReportBuilder {	
	...
	private final static Logger logger = Logger.getLogger(MiningScoopReportBuilder.class
			.getName());
	...
	public static void buildReport(String fullName, ArrayList<CommentDetails> comments) {
		...
		String ChartofDay = "\\\\t65-w7-eqcash\\incoming\\ChalkServer\\MiningScoopImages\\ChartofDay.png";
		String Inventories = "\\\\t65-w7-eqcash\\incoming\\ChalkServer\\MiningScoopImages\\Inventories.png";
		String MorningPriceChart = "\\\\t65-w7-eqcash\\incoming\\ChalkServer\\MiningScoopImages\\MorningPriceChart.png";
		...
		body = body.replace("{{ChartofDayImage}}", ChartofDay)
	...
	public static String buildBellComments(ArrayList<Comment> comments) {
		...
		for(Comment comment : comments) {
			if(comment.RIC().equals("BellComment") && comment.belongsTo() != null && comment.belongsTo().contains("Currencies")) { //indicates before the bell comment
				temp = bellComment;
				if (comment.belongsTo() == null)
				{
					temp = temp.replace("{{belongsTo}}", " ");
	...
	private static String getIOItradelist(String side)
	{
	    ...
	    try {
			...
			for (int i = 0; i < iois.length(); i++)
			{
				...
				if (!working && !retail && ticker != null && ric.endsWith(".TO") && !ric.contains("db.TO") && Utilities.isMaterialsTicker(ric))
				{
