...
public class EnergyInsightsReportBuilder {
  ...
  private static final Logger logger =
      Logger.getLogger(EnergyInsightsReportBuilder.class.getName());
    ...
    try {
      ...
      for (int i = 0; i < iois.length(); i++) {
        ...
        if (!working
            ...
            && ric.endsWith(".TO")
            && !ric.contains("db.TO")
            && Utilities.isEnergyUtilTicker(ric)) {
  ...
  public static ArrayList<String> buildSymbolComments(
      ArrayList<CommentDetails> comments, ArrayList<EnergyCommentDetails> ecomments) {
