...
public class MorningReportBuilder {
  ...
  private static final Logger logger = Logger.getLogger(MorningReportBuilder.class.getName());
  ...
  public static void buildReport(
    ...
    String energyStats =
        (eret >= 0 ? "<font color=\"#27AE60\">" : "<font color=\"#C0392B\">")
            + (eret >= 0 ? "+" : "")
            + String.format("%1$.2f", eret * 100)
            + "%"
            + "</font>, "
            + String.format("%1$.2f", evol)
            + "x ADV";
    String materialsStats =
        (mret >= 0 ? "<font color=\"#27AE60\">" : "<font color=\"#C0392B\">")
            + (mret >= 0 ? "+" : "")
            + String.format("%1$.2f", mret * 100)
            + "%"
            + "</font>, "
            + String.format("%1$.2f", mvol)
            + "x ADV";
    ...
    if (sector.equals("Other")) {
      ...
      for (CommentDetails comment : comments) {
        if (comment.sector().equals(sector)) { // indicates that this comment belongs to sector
          temp = macroComment;
          temp =
              temp.replace("{{belongsTo}}", comment.belongsTo())
                  .replace("{{evol}}", String.format("%1$.2f", comment.excessVol))
                  .replace(
                      "{{body}}",
                      (comment.sentiment() != null ? comment.sentiment() + ": " : "")
