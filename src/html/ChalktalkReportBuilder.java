...
public class ChalktalkReportBuilder {	
	...
	private final static Logger logger = Logger.getLogger(ChalktalkReportBuilder.class
			.getName());
	
	private static String name = "";

	
	public static void buildReport(String fullName, ArrayList<CommentDetails> comments, ArrayList<GeneralComment> generalComments, ArrayList<String> reportSections) {
...
//		String marbImage = "/images/Marb.png";
		...
		ArrayList<String> shortLongComments =  buildChalktalkSymbolComments(comments, Utilities.sectionIncluded(reportSections, "Research Highlights"), Utilities.sectionIncluded(reportSections, "Names in the News")); 
		...
		if (fullName.contains("George Gardiner")) {
			...
			revisionsImage = "\\\\t65-w7-eqcash\\incoming\\ChalkServer\\GeorgeChalktalkImages\\Revisions.png";
		}
	...
	public static String includeSelectedSections(ArrayList<String> reportSections, String htmlTemplate, ArrayList<Comment> comments ) {
		...
		if (!Utilities.sectionIncluded(reportSections, "Pertinent Revisions")) {
			...
			body = body.replace("{{PertinentRevisions}}", Utilities.getHTMLString("PertinentRevisions.html"));
		}
		...
		if (!Utilities.sectionIncluded(reportSections, "Research Highlights")) {
			...
			body = body.replace("{{ResearchHighlights}}", Utilities.getHTMLString("ResearchHighlights.html"));
		}
		...
		if (!Utilities.sectionIncluded(reportSections, "Before the Bell")) {
			...
			if (found)
				body = body.replace("{{BeforeBell}}", Utilities.getHTMLString("BeforeBell.html"));
			else
				body = body.replace("{{BeforeBell}}", "");	
		}
	...
	public static ArrayList<String> buildChalktalkSymbolComments(ArrayList<CommentDetails> comments, boolean includeHighlights, boolean includeDetailedComment) {
				...
				for(CommentDetails comment : comments) {
					...
					if(!comment.RIC().equals("")) { //symbol is present
						...
						if(comment.tickerResearch != null && 
								...
								!comment.tickerResearch.rating.equals("N/A"))
						...
						temp = temp
								...
								.replace("{{body}}",  Utilities.formatSentiment(comment) + Utilities.parseQuoteComment(comment.body(), false, true))
								.replace("{{rating}}", research.rating)
								.replace("{{target}}", research.target)
								.replace("{{researchLink}}", research.researchLink); 
						if (includeHighlights) {
							temp = temp.replace("{{linkHighlight}}", " <a href=\"#Research Highlights\" title=\"Back to Research Highlights\">Back</a>");
						}
						else {
				...
				for(CommentDetails comment : comments) {
					...
					if(!comment.RIC().equals("")) { //symbol is present
						...
						if (!comment.summary().equals("")) {
							...
							if (includeDetailedComment) {
								temp2 = temp2.replace("{{belongsTo}}", " <a href=\"#Our Take on {{belongsTo}}\" style=\"color:#5a5a5a; text-decoration: none;\" title=\"Click to read our take on {{belongsTo}}\">{{belongsTo}}</a>");
								temp2 = temp2.replace("{{body}}", " <a href=\"#Our Take on {{belongsTo}}\" style=\"color:#5a5a5a; text-decoration: none;\" title=\"Click to read our take on {{belongsTo}}\">{{body}}</a>");
								temp2 = temp2.replace("{{sentiment}}", " <a href=\"#Our Take on {{belongsTo}}\" style=\"text-decoration: none;\" title=\"Click to read our take on {{belongsTo}}\">{{sentiment}}</a>");
							}
							temp2 = temp2
									.replace("{{belongsTo}}", ticker)
									.replace("{{sentiment}}", Utilities.formatSentimentNoDash(comment))
									.replace("{{body}}",  Utilities.parseQuoteComment(comment.summary(), false, true));						
