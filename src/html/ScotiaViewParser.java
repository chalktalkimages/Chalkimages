...
public class ScotiaViewParser {
	...
	private final static Logger logger = Logger.getLogger(ScotiaViewParser.class
			.getName());
	...
	public ScotiaViewParser() {
 	...
 	private boolean getLoginCookies() {
		...
		try {
			...
			Connection.Response res = Jsoup.connect("https://www.scotiaview.com/cs/Satellite")
			...
			logger.info("Error in logging into Scotiaview after 5 attempts ... No symbol data will be provided: " + e.getMessage());
			return false;
		}		
	}
	...
	private boolean populateSecMap() {
		int attemptCounter = 0;
		try {
			Document SecuritiesJSONPage = Jsoup.connect("https://www.scotiaview.com/cs/Satellite?pagename=SCResearch/Search/JsonAutoSuggest&pcfs=0&requesttype=all")
			...
			//String SecuritiesJSONHtml = JSONHtml.substring(JSONHtml.indexOf("SC_Issuer")+12, JSONHtml.indexOf("SC_Industry")-5);
			String SecuritiesJSONHtml = JSONHtml.substring(JSONHtml.indexOf("SC_Issuer")+12, JSONHtml.length());
			//System.out.println(SecuritiesJSONHtml);
			
			String[] securitiesStrings = SecuritiesJSONHtml.split(Pattern.quote("##"));
			
//			HashMap<String, ArrayList<SecInfo>> tickerIDMap = new HashMap<String, ArrayList<SecInfo>>();
//			
			...
			logger.info("Error in getting security map from Scotiaview after 5 attempts ... no symbol data provided in report: " + e.getMessage());
			return false;
		}	
	}
	...
	private TickerResearch getPageDetails(String secID) {
		...
		int attemptCounter = 0;
		try {
			Document doc = Jsoup.connect("https://www.scotiaview.com/company/" + secID)
		               .userAgent("Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:25.0) Gecko/20100101 Firefox/25.0")
		               .referrer("http://www.google.com")  
		               .proxy("proxyprd.scotia-capital.com",8080)
		               .cookies(loginCookies)
		...
		if (rating.equals("N/A") && target.equals("N/A") && researchLink.equals(""))
			return null;
		else
			return new TickerResearch(rating, target, researchLink);
	}
	...
	public TickerResearch getSymbolResearch(String ric, String ticker) {
		...
		if (research.rating.equals(""))
			research.rating = "N/A";
		if (research.target.equals("1 yr:"))
			research.target = "N/A";			
	...
	private String getAltRIC(String ric)
	{
	    ...
	    try {  
            ...
            String cmdString = "SELECT [RIC] FROM [SecurityMaster].[dbo].[IEST_MSD_BASE] WHERE (([EQY_PRIM_EXCH] = 'CT' AND [REUTERS_SUFFIX] = 'TO') OR ([EQY_PRIM_EXCH] = 'CV' AND [REUTERS_SUFFIX] = 'V') OR ([EQY_PRIM_EXCH] = 'UN' AND [REUTERS_SUFFIX] = 'N') OR ([EQY_PRIM_EXCH] = 'UP' AND [REUTERS_SUFFIX] = 'P') OR ([EQY_PRIM_EXCH] = 'UQ' AND [REUTERS_SUFFIX] = 'O') OR ([EQY_PRIM_EXCH] = 'UR' AND [REUTERS_SUFFIX] = 'O') OR ([EQY_PRIM_EXCH] = 'UW' AND [REUTERS_SUFFIX] = 'O') OR ([EQY_PRIM_EXCH] = 'UA' AND [REUTERS_SUFFIX] = 'A')) AND [RIC] IS NOT NULL AND [ID_EXCH_SYMBOL] IS NOT NULL AND [ID_CUSIP]= (SELECT [ID_CUSIP] FROM [SecurityMaster].[dbo].[IEST_MSD_BASE] WHERE (([EQY_PRIM_EXCH] = 'CT' AND [REUTERS_SUFFIX] = 'TO') OR ([EQY_PRIM_EXCH] = 'CV' AND [REUTERS_SUFFIX] = 'V') OR ([EQY_PRIM_EXCH] = 'UN' AND [REUTERS_SUFFIX] = 'N') OR ([EQY_PRIM_EXCH] = 'UP' AND [REUTERS_SUFFIX] = 'P') OR ([EQY_PRIM_EXCH] = 'UQ' AND [REUTERS_SUFFIX] = 'O') OR ([EQY_PRIM_EXCH] = 'UR' AND [REUTERS_SUFFIX] = 'O') OR ([EQY_PRIM_EXCH] = 'UW' AND [REUTERS_SUFFIX] = 'O') OR ([EQY_PRIM_EXCH] = 'UA' AND [REUTERS_SUFFIX] = 'A')) AND [RIC] IS NOT NULL AND [ID_EXCH_SYMBOL] IS NOT NULL and RIC = " + "'" + ric + "')" + " and RIC != " + "'" + ric + "'";
		...
		try {
			...
			for (Element content : contents) {
				...
				if((rowType.equals("odd-row") || rowType.equals("even-row") )&& !header) {
					...
					for(Element e : es) {
						if(e.attr("href").contains("authors"))
							newStory.author = e.text();
						else if(e.attr("href").contains("company"))
							newStory.ticker = e.text();
					}
					es = content.getElementsByTag("p");
					if(es.size() >= 3)
						newStory.story = filterComment(es.get(2).text());
	...
	private String filterComment(String content) {
		...
		if(result.startsWith("Flash") || result.startsWith("Daily")) {
			...
			result = result.substring(result.indexOf("-")+1);
			result = result.replaceAll(".[a-zA-Z]{3} [0-9]{2}, [0-9]{4} [0-9:]{5}", ""); //remove date Jan 10, 2018 16:15
