...
public class ScotiaViewParser {
	...
	private static final Logger logger = Logger
			.getLogger(ScotiaViewParser.class.getName());
	...
	private boolean getLoginCookies() {
		...
		try {
			...
			Connection.Response res = Jsoup
					.connect("https://www.scotiaview.com/cs/Satellite")
					.userAgent(
							"Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2987.133 Safari/537.36")
					.method(Method.POST)
					.proxy("proxyprd.scotia-capital.com", 8080)
					.data("pagename", "SCResearch/Controllers/Login")
					.data("username", "bliu2108").data("password", "bliu2108")
					.header("X-Requested-With", "XMLHttpRequest")
					.timeout(15 * 1000).followRedirects(false).execute();
			...
			if (attemptCounter < attemptLimit) {
				attemptCounter++;
				return getLoginCookies(); // try again if this is the first
											// attempt
			}
			logger.info("Error in logging into Scotiaview after 5 attempts ... No symbol data will be provided: "
					+ e.getMessage());
			return false;
		}
	}
	...
	private boolean populateSecMap() {
		int attemptCounter = 0;
		try {
			Document SecuritiesJSONPage = Jsoup
					.connect(
					...
					.proxy("proxyprd.scotia-capital.com", 8080)
					.cookies(loginCookies).timeout(15 * 1000).get();
			String JSONHtml = SecuritiesJSONPage.html();
			// String SecuritiesJSONHtml =
			// JSONHtml.substring(JSONHtml.indexOf("SC_Issuer")+12,
			// JSONHtml.indexOf("SC_Industry")-5);
			String SecuritiesJSONHtml = JSONHtml.substring(
					JSONHtml.indexOf("SC_Issuer") + 12, JSONHtml.length());
			// System.out.println(SecuritiesJSONHtml);

			String[] securitiesStrings = SecuritiesJSONHtml.split(Pattern
					.quote("##"));

			// HashMap<String, ArrayList<SecInfo>> tickerIDMap = new
			// HashMap<String,
			// ArrayList<SecInfo>>();
			//
			for (int i = 0; i < securitiesStrings.length; i++) {
				String[] secDetails = securitiesStrings[i].split(Pattern
						.quote("||")); // two elements
				String[] secNameTicker = secDetails[0]
						.split(Pattern.quote("|"));
				String ticker;
				String exchange;
				String secID;
			...
			if (attemptCounter < attemptLimit) {
				attemptCounter++;
				return populateSecMap(); // try again if this is the first
											// attempt
			}
			logger.info("Error in getting security map from Scotiaview after 5 attempts ... no symbol data provided in report: "
					+ e.getMessage());
			return false;
		}
	}
	...
	private TickerResearch getPageDetails(String secID) {
		...
		try {
			Document doc = Jsoup
					.connect("https://www.scotiaview.com/company/" + secID)
					.userAgent(
							"Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:25.0) Gecko/20100101 Firefox/25.0")
					.referrer("http://www.google.com")
					.proxy("proxyprd.scotia-capital.com", 8080)
					.cookies(loginCookies).timeout(10 * 1000).get();
			...
			for (Element content : contents) {
				if (content.text().equals("Rating")) {
					...
					if (target.contains("1 yr: ")
							&& target.replace("1 yr: ", "").length() != 0) {
			...
			logger.info("Failed to get symbol data for ticker after 5 attempts "
					+ e.getMessage());
			return null;
		}

		if (rating.equals("N/A") && target.equals("N/A")
				&& researchLink.equals(""))
			return null;
		else
	...
	public TickerResearch getSymbolResearch(String ric, String ticker) {
		...
		if (research.target.equals("N/A") && loginsuccess && secMapsuccess) {
			...
			if (!altRIC.equals("")) {
				String altTicker = SymbolConverter.getPrefix(SymbolConverter
						.RIC2Symbol(altRIC));

				if (EquityFileParser.tickerResearchMap.containsKey(altTicker)) {
					altResearch = EquityFileParser.tickerResearchMap
							.get(altTicker);
				} else {
					altResearch = getResearch(altRIC, altTicker);
				}

				if (!altResearch.target.equals("N/A")
						|| !altResearch.researchLink.equals("")) {
	...
	public List<FlowStories> getStories(String url) {
		...
		try {
			...
			for (Element content : contents) {
				...
				if ((rowType.equals("odd-row") || rowType.equals("even-row"))
						&& !header) {
					String publishDateString = content.getElementsByClass(
							"align-rt").text();
					Calendar pubtime = Calendar.getInstance();
					pubtime.setTime(formatter.parse(publishDateString));
					if (pubtime.before(yesterday))
						continue; // skip old entries
					String pdfURL = content.getElementsByTag("a").get(1)
							.attr("href");
			...
			logger.info("Failed to get today's stories main page after 5 attempts "
					+ e.getMessage());
			return fs;
		}
		return fs;
	...
	private String filterComment(String content) {
		...
		if (result.startsWith("Flash") || result.startsWith("Daily")) {
			...
			result = result.replaceAll(
					".[a-zA-Z]{3} [0-9]{2}, [0-9]{4} [0-9:]{5}", ""); // remove
																		// date
																		// Jan
																		// 10,
																		// 2018
																		// 16:15
			result = result.replaceAll(".(\\w+) [0-9]{2}, [0-9]{4}", ""); // remove
																			// date
																			// January
																			// 11,
																			// 2018
