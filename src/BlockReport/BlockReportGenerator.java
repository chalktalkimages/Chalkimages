...
public class BlockReportGenerator {
  ...
  private static final Logger logger = Logger.getLogger(BlockReportGenerator.class.getName());
  ...
  public static String formatLargeNumber(long number, String prefix) {
    ...
    else if (num >= 1000) result = Math.round(num / 1000.0) + "K";
    else result = Long.toString(Math.round(num));
    if (!result.isEmpty()) {
  ...
  public static TreeMap<String, List<BlockAnalyst>> getBlocksByTrader(
      ArrayList<BlockDetails> blocks) {
    return getBlocksByMap(blocks, false);
  }

  public static TreeMap<String, List<BlockAnalyst>> getBlocksByAnalyst(
      ArrayList<BlockDetails> blocks) {
    return getBlocksByMap(blocks, true);
  }

  // groups by analyst or trader, broker, aggregate fiscal, year, month
  private static TreeMap<String, List<BlockAnalyst>> getBlocksByMap(
      ArrayList<BlockDetails> blocks, boolean isAnalyst) {
  ...
  private static Map<String, String> getScotiaRanking(
      ArrayList<BlockDetails> blocks, String category) {
    // a map with ticker as key and an inner map with brokerID and total trade volume for each
    // broker as value
  ...
  // groups by security
  public static Collection<BlockSecurity> getBlocksBySecurity(
      ArrayList<BlockDetails> blocks, java.lang.Boolean isFlow) {
        ...
        if (!secblockmap.containsKey(block.ticker)) {
          BlockSecurity bs =
              new BlockSecurity(
                  ...
                  block.tradeSize,
                  block.notional,
                  scotiaRankbyVolume.get(block.ticker),
                  scotiaRankbyValue.get(block.ticker));
  ...
  private static String[] generateATTable(
      Map<String, List<BlockAnalyst>> dataMap, String tableTemplate, String rowTemplate) {
    ...
    for (Entry<String, List<BlockAnalyst>> entry : dataMap.entrySet()) {
      ...
      Collections.sort(
          blocklist,
          new Comparator<BlockAnalyst>() {
            @Override
            public int compare(BlockAnalyst block1, BlockAnalyst block2) {
              return (block2.YTDnotional - block1.YTDnotional) > 0
                  ? 1
                  : ((block2.YTDnotional - block1.YTDnotional) < 0 ? -1 : 0);
            }
          });
      ...
      Collections.sort(
          ...
          new Comparator<BlockAnalyst>() {
            @Override
            public int compare(BlockAnalyst block1, BlockAnalyst block2) {
              return (block2.YTDshares - block1.YTDshares) > 0
                  ? 1
                  : ((block2.YTDshares - block1.YTDshares) < 0 ? -1 : 0);
            }
          });
      tablehtml = tablehtml.replace("{{atTableRow}}", formatATRow(blocklist, rowTemplate));
  ...
  public static void saveAnalystTraderEmailHtml(
      String fullname,
      Map<String, List<BlockAnalyst>> analystMap,
      Map<String, List<BlockAnalyst>> traderMap) {
  ...
  public static Map<String, Map<String, TraderSummary>> getMarketSharesByTrader(
      ArrayList<BlockDetails> blocks) throws Exception {
    ...
    Map<String, Map<String, TraderSummary>> traderMap =
        new HashMap<String, Map<String, TraderSummary>>();
  ...
  private static String formatTotalMarketRow(
      ArrayList<TraderTotal> traderTotal, String rowTemplate) {
    Integer count = 1;
    String rowshtml = "";
    for (TraderTotal tradertotal : traderTotal) {
      rowshtml +=
          rowTemplate
              .replace("{{tradername}}", tradertotal.traderName)
              .replace(
                  "{{blockvolume}}",
                  formatLargeNumber(tradertotal.marketTotalVolume.longValue(), ""))
              .replace("{{volumepercent}}", tradertotal.volumePercent + "%")
              .replace(
                  "{{blockvalue}}",
                  formatLargeNumber(tradertotal.marketTotalValue.longValue(), "$"))
  ...
  private static String formatScotiaVStotalRow(
      ArrayList<TraderTotal> traderTotal, String rowTemplate) {
    Integer count = 1;
    String rowshtml = "";
    for (TraderTotal tradertotal : traderTotal) {
      rowshtml +=
          rowTemplate
              ...
              .replace(
                  "{{scotiavolume}}",
                  formatLargeNumber(tradertotal.scotiaTotalVolume.longValue(), ""))
              .replace("{{scotiavolumepercent}}", tradertotal.scotiaVolumePercent + "%")
              .replace(
                  "{{scotiavalue}}",
                  formatLargeNumber(tradertotal.scotiaTotalValue.longValue(), "$"))
  ...
  private static String formatTraderPercentScotiaRow(
      ArrayList<TraderTotal> traderTotal, String rowTemplate) {
    Integer count = 1;
    String rowshtml = "";
    for (TraderTotal tradertotal : traderTotal) {
  ...
  private static Map<String, TraderTotal> generateTraderTotal(
      Map<String, Map<String, TraderSummary>> traderMap) {
    ...
    for (Entry<String, TraderTotal> traderTM : traderTotalMap.entrySet()) {
      ...
      if (marketVolume > 0) {
        tt.volumePercent =
            df.format(100 * tt.marketTotalVolume.doubleValue() / marketVolume.doubleValue());
      }
      if (marketValue > 0) {
        tt.valuePercent = df.format(100 * tt.marketTotalValue / marketValue);
      }
      if (tt.marketTotalVolume > 0) {
        tt.scotiaVolumePercent =
            df.format(
                100 * tt.scotiaTotalVolume.doubleValue() / tt.marketTotalVolume.doubleValue());
      }
      if (tt.marketTotalValue > 0) {
        tt.scotiaValuePercent = df.format(100 * tt.scotiaTotalValue / tt.marketTotalValue);
      }
      if (scotiaVolume > 0) {
        tt.volumePercentofScotia =
            df.format(100 * tt.scotiaTotalVolume.doubleValue() / scotiaVolume.doubleValue());
      }
      if (scotiaValue > 0) {
  ...
  private static String[] generateTraderSummaryTable(
      Map<String, Map<String, TraderSummary>> traderMap,
      String[] tableTemplates,
      String[] rowTemplates) {
        ...
        if (i == 0) {
          ...
          tablehtml =
              tableTemplate.replace(
                  "{{atTableRow}}", formatTotalMarketRow(traderList, rowTemplate));
          blockTableValue += tablehtml;
        } else if (i == 1) {
          String tablehtml = "";
          tablehtml =
              tableTemplate.replace(
                  "{{atTableRow}}", formatScotiaVStotalRow(traderList, rowTemplate));
          blockTableValue += tablehtml;
        } else {
          String tablehtml = "";
          tablehtml =
              tableTemplate.replace(
                  "{{atTableRow}}", formatTraderPercentScotiaRow(traderList, rowTemplate));
          blockTableValue += tablehtml;
        }
      }
      if (i == 3) {
        for (Entry<String, Map<String, TraderSummary>> tradermapsummary : traderMap.entrySet()) {
          ...
          for (Entry<String, TraderSummary> summary : summarymap.entrySet()) {
            ...
            singlesummary.ticker =
                SymbolConverter.getPrefix(SymbolConverter.RIC2Symbol(singlesummary.ticker));
          ...
          tablehtml =
              tablehtml.replace("{{atTableRow}}", formatMBDetailRow(summarylist, rowTemplate));
          blockTableValue += tablehtml;
        }
      }
  ...
  public static void saveMarketShareSummaryEmailHtml(
      String fullname, Map<String, Map<String, TraderSummary>> traderMap) throws Exception {

    String fileSave = "BlockReportEmail.html";
    String formattedDate = (new SimpleDateFormat("EEEE, MMMM d, yyyy")).format(new Date());
    String reportHtml = Utilities.getHTMLString("MarketBlockReportTemplate.html");
    String[] tableSummaryTemplates =
        new String[] {
          Utilities.getHTMLString("MarketBlockSummaryTotalTable.html"),
          Utilities.getHTMLString("MarketBlockSummaryVSTable.html"),
          Utilities.getHTMLString("MarketBlockSummaryPercentTable.html"),
          Utilities.getHTMLString("MarketBlockDetailTable.html")
        };

    String[] rowSummaryTemplates =
        new String[] {
          Utilities.getHTMLString("MarketBlockSummaryTotalRow.html"),
          Utilities.getHTMLString("MarketBlockSummaryVSRow.html"),
          Utilities.getHTMLString("MarketBlockSummaryPercentRow.html"),
          Utilities.getHTMLString("MarketBlockDetailRow.html")
        };

    String[] traderSummaryTables =
        generateTraderSummaryTable(traderMap, tableSummaryTemplates, rowSummaryTemplates);
  ...
  public static void saveBlockEmailHtml(
    ...
    Collections.sort(
        blockbrokerList,
        new Comparator<BlockBroker>() {
          @Override
          public int compare(BlockBroker block1, BlockBroker block2) {
            return (block2.notional - block1.notional) > 0
                ? 1
                : ((block2.notional - block1.notional) < 0 ? -1 : 0);
          }
        });
    Integer count = 1;
    for (BlockBroker b : blockbrokerList) {
      ...
      rowshtml +=
          rowTemplate
              .replace("{{rank}}", count.toString())
              .replace("{{name}}", b.name)
              .replace("{{numtrades}}", b.blockCount.toString())
              .replace("{{volume}}", NumberFormat.getNumberInstance(Locale.US).format(b.shares))
    ...
    Collections.sort(
        ...
        new Comparator<BlockBroker>() {
          @Override
          public int compare(BlockBroker block1, BlockBroker block2) {
            return (block2.shares - block1.shares) > 0
                ? 1
                : ((block2.shares - block1.shares) < 0 ? -1 : 0);
          }
        });
    count = 1;
    for (BlockBroker b : blockbrokerList) {
    ...
    for (BlockSecurity b : blocksecurityList) {
      ...
      rowshtml +=
          rowTemplate1
              .replace("{{rank}}", count.toString())
              .replace("{{name}}", b.ticker)
              .replace("{{numtrades}}", b.blockCount.toString())
              .replace("{{scotiaRank}}", b.scotiaRankbyValue)
    ...
    Collections.sort(
        ...
        new Comparator<BlockSecurity>() {
          @Override
          public int compare(BlockSecurity block1, BlockSecurity block2) {
            return (block2.volume - block1.volume) > 0
                ? 1
                : ((block2.volume - block1.volume) < 0 ? -1 : 0);
          }
        });
    count = 1;
    for (BlockSecurity b : blocksecurityList) {
      ...
      rowshtml +=
          rowTemplate1
              ...
              .replace("{{name}}", b.ticker)
              .replace("{{numtrades}}", b.blockCount.toString())
              .replace("{{scotiaRank}}", b.scotiaRankbyVolume)
              .replace("{{volume}}", NumberFormat.getNumberInstance(Locale.US).format(b.volume))
              .replace("{{value}}", "$" + String.format("%1$,.0f", b.notional));
      count++;
    }
  ...
  public static void createR305Excel(
      Collection<BlockBroker> blockbroker,
      Collection<BlockSecurity> blocksecurity,
      HashMap<String, ArrayList<BlockDetails>> blockmap) {}

  public static void createBlockReportExcel(
      Collection<BlockBroker> blockbroker,
      Collection<BlockSecurity> blocksecurity,
      HashMap<String, ArrayList<BlockDetails>> blockmap) {
      ...
      for (Integer key : BrokerIDs) {
        ...
        for (BlockDetails block : blockList) {
          ...
          String time =
              new java.text.SimpleDateFormat("HH:mm:ss")
                  .format(new java.util.Date(block.tradeTime));
          ...
          label =
              new Label(
                  2,
                  rownumber,
                  NumberFormat.getNumberInstance(Locale.US).format(block.tradeSize),
                  CellFormat);
          excelSheet.addCell(label);
          label = new Label(3, rownumber, "$" + String.format("%.2f", price), CellFormat);
          excelSheet.addCell(label);
          label =
              new Label(4, rownumber, "$" + String.format("%1$,.0f", block.notional), CellFormat);
          excelSheet.addCell(label);
        ...
        label =
            new Label(
                2,
                rownumber,
                NumberFormat.getNumberInstance(Locale.US).format(cumVol),
                SummaryRowFormat);
        excelSheet.addCell(label);
  ...
  public static HashMap<String, ArrayList<BlockDetails>> sortbyBrokerTimeTicker(
      ArrayList<BlockDetails> blocks) {

    HashMap<String, ArrayList<BlockDetails>> blockbyTimeBroker =
        new HashMap<String, ArrayList<BlockDetails>>();
    ...
    for (String key : blockbyTimeBroker.keySet()) {
      ...
      Collections.sort(
          blockList,
          new Comparator<BlockDetails>() {
            @Override
            public int compare(BlockDetails block2, BlockDetails block1) {
              return (block2.tradeTime - block1.tradeTime) > 0
                  ? 1
                  : ((block2.tradeTime - block1.tradeTime) < 0 ? -1 : 0);
            }
          });
      Collections.sort(
          blockList,
